...
World.prototype.ballCollidingWithBrick = function () {
  return this.circle.x > this.bricks.x && this.circle.x < this.bricks.x + this.bricks.width && this.circle.y > this.bricks.y && this.circle.y < this.bricks.y + this.bricks.height;
};
...
World.prototype.collisionBricks = function(column, row) {
  for(column = 0; column < this.bricks.columnCount; column++) {
    for(row = 0; row < this.bricks.rowCount; row++) {
      if (this.ballCollidingWithBrick()) {
